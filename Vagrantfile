...
Vagrant.configure(2) do |config|
  ...
  config.vm.define :linux do |config|
    ...
    config.vm.synced_folder ".", "/vagrant", type: "nfs", mount_options: ['rw', 'vers=3', 'tcp', 'fsc', 'actimeo=2', 'nolock']
  ...
  config.vm.network "forwarded_port", guest: 27017, host: 2227, auto_correct: true
  config.vm.network "private_network", ip: "192.168.1.10"
  config.vm.hostname = "lemondeensac.local"
